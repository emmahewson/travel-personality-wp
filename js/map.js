function initMap(index) {
    ...
    // Setting WordPress image source for to be reset by click markers
    const imageOneOriginalSrc = imageOne.src
    const imageTwoOriginalSrc = imageTwo.src
    const imageThreeOriginalSrc = imageThree.src
    const imageFourOriginalSrc = imageFour.src
    ...
    for (let i = 0; i < countries[index].highlights.length; i++) {
        ...
        (function (marker, data) {
            google.maps.event.addListener(marker, "click", function (e) {
                ...
                imageOne.src = imageOneOriginalSrc + `img/${this.images[0].img}`;
                imageOne.alt = this.images[0].alt;
                imageTwo.src = imageTwoOriginalSrc + `img/${this.images[1].img}`;
                imageTwo.alt = this.images[1].alt;
                imageThree.src = imageThreeOriginalSrc + `img/${this.images[2].img}`;
                imageThree.alt = this.images[2].alt;
                imageFour.src = imageFourOriginalSrc + `img/${this.images[3].img}`;
